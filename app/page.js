                ...
                onChange={async (e)=>{
                  ...
                  if(e.label.toString().toLowerCase().includes('vet') && veWallet == null){
                      ...
                      try{
                        ...
                        setVeWallet(wallet); //todo should delete
                        console.log("wallet..",wallet);
                        let account = await wallet.getAccounts();
                        console.log("wallet.. after getAccounts",wallet);
                        console.log("vechain account",account[0],account);
                        console.log("wallet... select chain 1",wallet);
                        //setVeWallet(wallet); //todo should add
                        console.log("wallet... select chain 2",wallet);
                      }catch(err){
                        console.log(err);
                      }
                  }else{
                      console.log("veWallet select chain",veWallet);
                      if(veWallet){
...
export default function Home() {
  ...
  return (
      <Container maxWidth="lg" className={styles.container}>
                      ...
                      <TableContainer style={{maxHeight: 300, overflow: 'auto'}}>
                        <Table stickyHeader>
                          <TableHead>
                            ...
                            <TableRow>
                              <StyledTableHeaderCell><input value={filters['TokenPairID']}
                                                            placeholder='Filter by TokenPairID'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['TokenPairID']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['AncestorSymbol']}
                                                            placeholder='Filter by AncestorSymbol'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['AncestorSymbol']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['AncestorDecimals']}
                                                            placeholder='Filter by AncestorDecimals'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['AncestorDecimals']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['AncestorChainID']}
                                                            placeholder='Filter by AncestorChainID'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['AncestorChainID']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['FromChainID']}
                                                            placeholder='Filter by FromChainID'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['FromChainID']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['ToChainID']}
                                                            placeholder='Filter by ToChainID'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['ToChainID']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['AncestorAccount']}
                                                            placeholder='Filter by AncestorAccount'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['AncestorAccount']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['FromAccount']}
                                                            placeholder='Filter by FromAccount'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                                              ['FromAccount']: e.target.value
                                                            }))}/></StyledTableHeaderCell>
                              <StyledTableHeaderCell><input value={filters['ToAccount']}
                                                            placeholder='Filter by ToAccount'
                                                            onChange={e => setFilters(pre => ({
                                                              ...pre,
                                ...
                                <TableRow key={row.id} sx={{
                                  ...
                                  <StyledTableCell component="th" scope="row" style={{
                                    ...
                                    <Stack spacing={2} direction='row'>
                                      <div>{row.id}</div>
                                      <DraggableItem id={'tokenPair_' + row.id}>⭐</DraggableItem></Stack>
                                  </StyledTableCell>
                                  <StyledTableCell style={{
                                    ...
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.ancestorSymbol}</StyledTableCell>
                                  <StyledTableCell style={{
                                    maxWidth: 60,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.ancestorDecimals}</StyledTableCell>
                                  <StyledTableCell style={{
                                    minWidth: 150,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.ancestorChainID}<Chip size="small"
                                                                label={chains.find(v => Number(v.chainID) === Number(row.ancestorChainID))?.chainType}/></StyledTableCell>
                                  <StyledTableCell style={{
                                    minWidth: 150,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.fromChainID}<Chip size="small"
                                                            label={chains.find(v => Number(v.chainID) === Number(row.fromChainID))?.chainType}/></StyledTableCell>
                                  <StyledTableCell style={{
                                    minWidth: 150,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.toChainID}<Chip size="small"
                                                          label={chains.find(v => Number(v.chainID) === Number(row.toChainID))?.chainType}/></StyledTableCell>
                                  <StyledTableCell style={{
                                    maxWidth: 380,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.ancestorAccount}</StyledTableCell>
                                  <StyledTableCell style={{
                                    maxWidth: 380,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.fromAccount}</StyledTableCell>
                                  <StyledTableCell style={{
                                    maxWidth: 380,
                                    overflow: 'hidden',
                                    whiteSpace: 'nowrap',
                                    textOverflow: 'ellipsis'
                                  }}>{row.toAccount}</StyledTableCell>
                                </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                  ...
                  {loading ? (
                      ...
                      <>
                        <Stack spacing={1} direction='row'>
                          ...
                          <Button size='small' style={{marginBottom: '10px', textTransform: 'none'}} variant='outlined'
                                  onClick={() => setShowAddToken(true)}>Add Token...</Button>
                        </Stack>
                        <Card style={{borderRadius: 8, overflow: 'hidden'}}>
                          <TableContainer style={{maxHeight: 300, overflow: 'auto'}}>
                            <Table stickyHeader>
                              <TableHead>
                                ...
                                <TableRow>
                                  <StyledTableHeaderCell><input value={filters['Chain']} placeholder='Filter by Chain'
                                                                onChange={e => setFilters(pre => ({
                                                                  ...pre,
                                                                  ['Chain']: e.target.value
                                                                }))}/></StyledTableHeaderCell>
                                  <StyledTableHeaderCell><input value={filters['Symbol']} placeholder='Filter by Symbol'
                                                                onChange={e => setFilters(pre => ({
                                                                  ...pre,
                                                                  ['Symbol']: e.target.value
                                                                }))}/></StyledTableHeaderCell>
                                  <StyledTableHeaderCell><input value={filters['Name']} placeholder='Filter by Name'
                                                                onChange={e => setFilters(pre => ({
                                                                  ...pre,
                                                                  ['Name']: e.target.value
                                                                }))}/></StyledTableHeaderCell>
                                  <StyledTableHeaderCell><input value={filters['Decimals']}
                                                                placeholder='Filter by Decimals'
                                                                onChange={e => setFilters(pre => ({
                                                                  ...pre,
                                                                  ['Decimals']: e.target.value
                                                                }))}/></StyledTableHeaderCell>
                                  <StyledTableHeaderCell><input value={filters['Address']}
                                    ...
                                    <TableRow key={row} sx={{
                                      ...
                                      <StyledTableCell style={{
                                        ...
                                        textOverflow: 'ellipsis'
                                               ...
                                               label={chains.find(v => Number(v.chainID) === Number(tokens[row].chainID))?.chainType}/></StyledTableCell>
                                      <StyledTableCell component="th" scope="row" style={{
                                        maxWidth: 120,
                                        overflow: 'hidden',
                                        whiteSpace: 'nowrap',
                                        ...
                                        <Stack spacing={2} direction='row'>
                                          <DraggableItem
                                              id={'token,' + tokens[row].address + ',' + tokens[row].name + ',' + tokens[row].symbol + ',' + tokens[row].decimals + ',' + tokens[row].chainID}>⭐</DraggableItem>
                                      ...
                                      <StyledTableCell style={{
                                        ...
                                        whiteSpace: 'nowrap',
                                        textOverflow: 'ellipsis'
                                      }}>{tokens[row].name}</StyledTableCell>
                                      <StyledTableCell style={{
                                        maxWidth: 30,
                                        overflow: 'hidden',
                                        whiteSpace: 'nowrap',
                                        textOverflow: 'ellipsis'
                                      }}>{tokens[row].decimals}</StyledTableCell>
                                      <StyledTableCell style={{
                                        maxWidth: 380,
                                        overflow: 'hidden',
                                        whiteSpace: 'nowrap',
                                        textOverflow: 'ellipsis'
                                      }}>{tokens[row].address}</StyledTableCell>
                                    </TableRow>
                                ))}
                              </TableBody>
                            </Table>
              ...
              <Grid item xs={7}>
                <Paper elevation={3} className={styles.rightPart}>
                  <Stack spacing={1} direction='row' className={styles.buttonPanel}>
                    ...
                    <Button size='small' style={{marginBottom: '10px', textTransform: 'none'}} variant='outlined'
                            onClick={() => setCurrentPairs([])}>Clear</Button>
                    <Button size='small' style={{marginBottom: '10px', textTransform: 'none'}} variant='outlined'
                            onClick={async () => {
                              if (connected) {
                                setAddress('');
                  ...
                  <div className={styles.scrollContent}>
                    <DropZone width="98%" height="88%" placeholder="Drag and Drop Token Pair ⭐ Here to Modify"
                              onDrop={onPairDrop} tokenPairs={tokenPairs} currentPairs={currentPairs}/>
                    {
                        currentPairs.length > 0 && currentPairs.map((v, i) => {
                          return <NewPair ethers={ethers} key={JSON.stringify(v)} pair={v} tokens={tokens} chains={chains}
                                          updatePairId={(oldId, id) => {
                                            setCurrentPairs((pre) => {
        ...
        {
            showAddToken && (
                <Dialog open={showAddToken} onClose={() => setShowAddToken(false)} aria-labelledby="form-dialog-title"
                        sx={{borderRadius: 10}}>
                  ...
                  <DialogContent>
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12}>
                        <TextField margin="dense" id="symbol" label="Symbol" type="text" fullWidth value={addTokenSymbol}
                                   onChange={e => setAddTokenSymbol(e.target.value)}/>
                      </Grid>
                      <Grid item xs={12}>
                        <TextField margin="dense" id="name" label="Name" type="text" fullWidth value={addTokenName}
                                   onChange={e => setAddTokenName(e.target.value)}/>
                      </Grid>
                      <Grid item xs={12}>
                        <TextField margin="dense" id="decimals" label="Decimals" type="text" fullWidth
                                   value={addTokenDecimals} onChange={e => setAddTokenDecimals(e.target.value)}/>
                      </Grid>
                      <Grid item xs={12}>
                        <TextField margin="dense" id="address" label="Address" type="text" fullWidth value={addTokenAddress}
